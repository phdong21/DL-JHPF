...
for filename in filedir:
    ...
    for i in range(data_num_file):
        for j in range(fre):
            a=channel_norm_factor*channel[:,:,j,i]
...
# load model
model.load_weights('DLJHPF_UMi_3path_1fre_Ns3_10dB_500ep.hdf5')

# checkpoint
filepath='DLJHPF_UMi_3path_1fre_Ns3_10dB_600ep.hdf5'

checkpoint = ModelCheckpoint(filepath, monitor='val_loss', verbose=1, save_best_only=True, mode='min', save_weights_only=True)
callbacks_list = [checkpoint]

model.fit(x=[mod_symb, H_train1, H_train2, H_train2, channel_noise],y=trans_bit_label,epochs=100, batch_size=256, callbacks=callbacks_list, verbose=2, shuffle=True, validation_split=0.1)
